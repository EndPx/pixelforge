...
export interface EditorState {
  ...
  brushSize: number;
  brushShape: "square" | "circle" | "line";
  setBrushShape: (shape: "square" | "circle" | "line") => void;
  gridVisible: boolean;
  onionSkin: boolean;
  tiledMode: boolean;
  tool: ToolId;
  ...
  duplicateLayer: (opts?: { actor?: Actor }) => string | undefined;
...
export function createEditorStore(width = 32, height = 32) {
  ...
  return create<EditorState>()((setState, get) => {
    ...
    return {
      ...
      // Select the bounding box of the contiguous non-transparent blob under (x, y)
      selectBlob: (x, y) => {
        const s = get();
      ...
      setBrushSize: (size) =>
        setState((s) => ({ ...s, brushSize: Math.max(1, Math.min(64, Math.round(size))) })),
      setBrushShape: (shape) => setState((s) => ({ ...s, brushShape: shape })),
      ...
      // LibreSprite-level zoom: 6400% max (zoom = 1024 at 100% = 16)
      setZoom: (zoom) =>
